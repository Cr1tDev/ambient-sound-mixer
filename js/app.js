...
import { PresetManager } from "./presentManager.js";
import { Timer } from "./timer.js";

class AmbientMixer {
  // Initiallize dependencies and default state
  constructor() {
    this.soundManager = new SoundManager();
    this.ui = new UI();
    this.presetManger = new PresetManager();
  ...
  init() {
    try {
      ...
      console.error("Failed to initailize app: ", error);
    }
  }

  // Setup all event listeners
  setupEventListeners() {
    // Handle all clicks and event delegation
  ...
  loadAllSounds() {
    sounds.forEach((sound) => {
      ...
      if (!success) {
        console.warm(`Could not load sound: ${sound.name} from ${audioUrl}`);
      }
    });
  }
  ...
  async toggleSound(soundId) {
    ...
    if (!audio) {
      console.erorr(`Sound ${soundId} not found.`);
    }
  ...
  setMasterVolume(volume) {
    ...
    // Appy master volume to all currently palying sounds
  ...
  // Reset everyting to default state
  resetAll() {
    // Stop all sounds
  ...
  loadPreset(presetKey, custom = false) {
    ...
    if (!preset) {
      console.log(`Preset ${presetKey} not found.`);
      return;
    }
